...
public class LeaveDAO {
    public void apply(LeaveRequest l) {
        ...
        try (Connection c = DBConnection.getConnection(); PreparedStatement p = c.prepareStatement(s)) {
            p.setInt(1, l.employeeId());
            p.setDate(2, Date.valueOf(l.startDate()));
            p.setDate(3, Date.valueOf(l.endDate()));
            p.setString(4, l.reason());
            p.setString(5, l.status());
            p.executeUpdate();
        } catch (SQLException x) {
            throw new DatabaseException("Unable to apply leave.", x);
        }
    }
}
