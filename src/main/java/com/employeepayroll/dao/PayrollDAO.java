...
public class PayrollDAO {
    public void save(int id,String month,double base,double allowance,double deduction,double net){
        ...
        try(Connection c=DBConnection.getConnection();PreparedStatement p=c.prepareStatement(s)){
            p.setInt(1,id);p.setString(2,month);p.setDouble(3,base);p.setDouble(4,allowance);p.setDouble(5,deduction);p.setDouble(6,net);p.executeUpdate();
        }catch(SQLException x){throw new DatabaseException("Unable to save payroll.",x);}
    }
}
