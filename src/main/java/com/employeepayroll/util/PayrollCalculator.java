...
public final class PayrollCalculator {
    ...
    public static double calculate(Payable p){return Math.max(0,p.calculateNetSalary());}
    public static double calculateAttendanceAdjusted(Employee e,int present,int working){
        if(working<=0) throw new IllegalArgumentException("Working days must be greater than zero.");
